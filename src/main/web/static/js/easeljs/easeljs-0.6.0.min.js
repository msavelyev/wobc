...
k.regX);g&&(n.y=l.height-n.y-n.height,p.regY=n.height-k.regY);f.push(p)}m="_"+(m?"h":"")+(g?"v":"");g=b._animations;b=b._data;d=g.length/a;for(j=0;j<d;j++){f=g[j];k=b[f];h={name:f+m,frequency:k.frequency,next:k.next,frames:[]};k.next&&(h.next+=m);f=k.brickFrames;k=0;for(l=f.length;k<l;k++)h.brickFrames.push(f[k]+e*a);b[h.name]=h;g.push(h.name)}};createjs.SpriteSheetUtils=c})();this.createjs=this.createjs||{};
